...
import { setOpen } from "../redux/appSlice"; // ✅ import setOpen

const Sidebar = () => {
  const dispatch = useDispatch(); // ✅ correctly assign dispatch
  const sidebarItems = [
  ...
  return (
    <div className="w-[15%]">
      <div className="p-3">
        <button
          onClick={() => dispatch(setOpen(true))} // ✅ fixed onClick
      ...
      <div className="text-gray-500">
        {sidebarItems.map((item, index) => {
          return (
            <div
              key={index}
              className="flex items-center pl-6 py-1 gap-4 rounded-r-full my-2 cursor-pointer hover:bg-gray-200"
            >
              {item.icon}
              <p>{item.text}</p>
            </div>
